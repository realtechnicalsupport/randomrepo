...
function App() {
  ...
  const [syncToken, setSyncToken] = useState(() => { try { return localStorage.getItem("med_gh_token") || ""; } catch { return ""; } });
  const [anthropicKey, setAnthropicKey] = useState(() => { try { return localStorage.getItem("med_ai_key") || ""; } catch { return ""; } });
  const [gistId, setGistId] = useState(() => { try { return localStorage.getItem("med_gist_id") || ""; } catch { return ""; } });
  const [syncStatus, setSyncStatus] = useState("idle"); // idle | syncing | saved | error
  const [showSyncPanel, setShowSyncPanel] = useState(false);
  const syncTimer = useRef(null);

  const GIST_FILENAME = "medschool-progress.json";

  const pushToGist = useCallback(async (token, id, data) => {
    if (!token) return;
    setSyncStatus("syncing");
    try {
      let url = "https://api.github.com/gists";
      let method = "POST";
      let body = { description: "MedSchool USMLE Progress", public: false, files: { [GIST_FILENAME]: { content: JSON.stringify(data, null, 2) } } };
      if (id) { url = `https://api.github.com/gists/${id}`; method = "PATCH"; }
      ...
      if (!id) { const newId = json.id; setGistId(newId); try { localStorage.setItem("med_gist_id", newId); } catch {} }
      setSyncStatus("saved");
      setTimeout(() => setSyncStatus("idle"), 2000);
    } catch (e) { setSyncStatus("error"); }
  }, []);

  const pullFromGist = useCallback(async (token, id) => {
  ...
  // On mount, pull latest data from Gist
  useEffect(() => {
    if (!syncToken || !gistId) return;
    pullFromGist(syncToken, gistId).then(data => {
      if (!data) return;
      if (data.completed) setCompleted(data.completed);
      if (data.recallItems) setRecallItems(data.recallItems);
      if (data.seenTerms) setSeenTerms(data.seenTerms);
    });
  }, []); // eslint-disable-line
  ...
  useEffect(() => {
    ...
    syncTimer.current = setTimeout(() => {
      pushToGist(syncToken, gistId, { completed, recallItems, seenTerms, updatedAt: new Date().toISOString() });
    }, 2000);
    return () => clearTimeout(syncTimer.current);
  }, [completed, recallItems, seenTerms, syncToken, gistId, pushToGist]);
  ...
  const saveToken = (token) => {
    setSyncToken(token);
    try { localStorage.setItem("med_gh_token", token); } catch {}
    if (token) pushToGist(token, gistId, { completed, recallItems, seenTerms, updatedAt: new Date().toISOString() });
  };

  const saveAnthropicKey = (key) => {
    setAnthropicKey(key);
    try { localStorage.setItem("med_ai_key", key); } catch {}
  };

  const SyncPanel = () => (
    <div style={{ position: "fixed", inset: 0, zIndex: 200, display: "flex", alignItems: "flex-end", justifyContent: "center" }} onClick={() => setShowSyncPanel(false)}>
      <div style={{ position: "absolute", inset: 0, background: "rgba(0,0,0,0.4)" }} />
      <div onClick={e => e.stopPropagation()} style={{ position: "relative", background: T.surface, borderRadius: "16px 16px 0 0", padding: "24px 20px 40px", width: "100%", maxWidth: 560, maxHeight: "85vh", overflowY: "auto" }}>
        <div style={{ width: 36, height: 4, background: T.border, borderRadius: 2, margin: "0 auto 20px" }} />
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
          <div style={{ fontFamily: T.font, fontSize: 18, fontWeight: 700, color: T.ink }}>Settings</div>
          <div style={{ fontFamily: T.font, fontSize: 11, fontWeight: 700, color: T.inkLight, background: T.bgDeep, padding: "3px 8px", borderRadius: 4 }}>v{APP_VERSION}</div>
        </div>

        {/* Anthropic API Key */}
        <div style={{ marginBottom: 24, padding: "16px", background: T.bgDeep, borderRadius: 10, border: `1px solid ${T.border}` }}>
          <div style={{ fontFamily: T.font, fontSize: 15, fontWeight: 700, color: T.ink, marginBottom: 4 }}>🤖 Anthropic API Key</div>
          <div style={{ fontFamily: T.font, fontSize: 12, fontWeight: 600, color: T.inkLight, lineHeight: 1.6, marginBottom: 12 }}>
            Required for Practice Quiz, AI Professor, and Review questions. Get one free at <span style={{ color: T.blue }}>console.anthropic.com</span> → API Keys.
          </div>
          <input
            type="password"
            placeholder="sk-ant-…"
            defaultValue={anthropicKey}
            onBlur={e => saveAnthropicKey(e.target.value.trim())}
            style={{ width: "100%", padding: "11px 14px", fontFamily: T.font, fontSize: 14, fontWeight: 600, border: `1.5px solid ${anthropicKey ? T.sage : T.border}`, borderRadius: 8, background: T.surface, color: T.ink, outline: "none" }}
          />
          {anthropicKey && <div style={{ fontFamily: T.font, fontSize: 12, fontWeight: 700, color: T.sage, marginTop: 6 }}>✓ API key saved</div>}
        </div>

        {/* GitHub Sync */}
        <div style={{ marginBottom: 16, padding: "16px", background: T.bgDeep, borderRadius: 10, border: `1px solid ${T.border}` }}>
          <div style={{ fontFamily: T.font, fontSize: 15, fontWeight: 700, color: T.ink, marginBottom: 4 }}>☁️ GitHub Sync</div>
          <div style={{ fontFamily: T.font, fontSize: 12, fontWeight: 600, color: T.inkLight, lineHeight: 1.6, marginBottom: 12 }}>
            Syncs progress across devices via a private Gist. Go to <span style={{ color: T.blue }}>github.com/settings/tokens</span> → Generate new token (classic) → tick <b>gist</b> only.
          </div>
          <input
            type="password"
            placeholder="GitHub personal access token…"
            defaultValue={syncToken}
            onBlur={e => saveToken(e.target.value.trim())}
            style={{ width: "100%", padding: "11px 14px", fontFamily: T.font, fontSize: 14, fontWeight: 600, border: `1.5px solid ${syncToken ? T.sage : T.border}`, borderRadius: 8, background: T.surface, color: T.ink, outline: "none", marginBottom: 8 }}
          />
          {syncToken && (
            <div style={{ fontFamily: T.font, fontSize: 12, fontWeight: 700, color: syncStatus === "error" ? T.clay : syncStatus === "saved" ? T.sage : T.inkLight, marginBottom: 10 }}>
              {syncStatus === "syncing" ? "⟳ Syncing…" : syncStatus === "saved" ? "✓ Saved to Gist" : syncStatus === "error" ? "✗ Sync failed — check token" : gistId ? `✓ Connected · Gist ${gistId.slice(0,8)}…` : "Ready to sync"}
            </div>
          )}
          {syncToken && (
            <button onClick={async () => { const data = await pullFromGist(syncToken, gistId); if (data) { if (data.completed) setCompleted(data.completed); if (data.recallItems) setRecallItems(data.recallItems); if (data.seenTerms) setSeenTerms(data.seenTerms); setSyncStatus("saved"); setTimeout(() => setSyncStatus("idle"), 2000); } }} style={{ width: "100%", background: T.sage, color: "#fff", border: "none", borderRadius: 8, padding: "11px 0", fontFamily: T.font, fontSize: 14, fontWeight: 700, cursor: "pointer" }}>
              ↓ Pull latest from Gist
            </button>
          )}
        </div>

        <button onClick={() => setShowSyncPanel(false)} style={{ width: "100%", background: T.bgDeep, color: T.inkMid, border: "none", borderRadius: 8, padding: "12px 0", fontFamily: T.font, fontSize: 14, fontWeight: 600, cursor: "pointer" }}>Close</button>
      </div>
    </div>
  );
  ...
  useEffect(() => { document.body.style.overflow = (sidebarOpen || dictOpen || activePopup || showSyncPanel) ? "hidden" : ""; return () => { document.body.style.overflow = ""; }; }, [sidebarOpen, dictOpen, activePopup, showSyncPanel]);
  ...
  const generatePracticeQuiz = async () => {
    ...
    try {
      ...
      setPracticeError(e.message || "Unknown error");
    }
    ...
    <div style={{ display: "flex", flexDirection: "column", height: "100dvh", background: T.bg, color: T.ink, fontFamily: T.font, overflow: "hidden", position: "relative" }}>
      ...
      <div style={{ background: T.surface, borderBottom: `1px solid ${T.border}`, padding: "10px 12px", display: "flex", alignItems: "center", gap: 8, flexShrink: 0, zIndex: 10 }}>
        ...
        <div style={{ display: "flex", gap: 6 }}>
          ...
          <button onClick={() => setShowSyncPanel(true)} style={{ background: syncToken ? T.sageSoft : T.bgDeep, border: "none", borderRadius: 8, padding: "6px 10px", fontFamily: T.font, fontSize: 12, fontWeight: 700, color: syncToken ? T.sage : T.inkLight, cursor: "pointer", flexShrink: 0 }} title={`Sync & Settings · v${APP_VERSION}`}>
            {syncStatus === "syncing" ? "⟳" : syncStatus === "saved" ? "✓" : syncStatus === "error" ? "✗" : "☁"} <span style={{ fontSize: 10, opacity: 0.7 }}>v{APP_VERSION}</span>
          </button>
        </div>
      </div>
      ...
      <div style={{ flex: 1, overflowY: "auto", WebkitOverflowScrolling: "touch" }}>
            ...
            <div style={{ flex: 1, overflowY: "auto", padding: "0 16px", WebkitOverflowScrolling: "touch" }}>
              {chat.length === 0 && (
                <div style={{ padding: "16px 0" }}>
                  {!anthropicKey && (
                    <div style={{ background: T.claySoft, border: `1px solid ${T.clay}`, borderRadius: 8, padding: "12px 14px", marginBottom: 16 }}>
                      <div style={{ fontFamily: T.font, fontSize: 13, fontWeight: 700, color: T.clay, marginBottom: 4 }}>API key required</div>
                      <div style={{ fontFamily: T.font, fontSize: 13, fontWeight: 600, color: T.inkMid, lineHeight: 1.6 }}>Tap the <b>☁</b> button (top right) and paste your Anthropic API key to enable the Professor.</div>
                    </div>
                  )}
        ...
        {view === "practice" && (
          <div className="fade" style={{ maxWidth: 680, margin: "0 auto", padding: "24px 16px 80px" }}>
            ...
            {/* Loading */}
            {!anthropicKey && completed.length > 0 && !practiceLoading && !practiceQuiz && (
              <div style={{ background: T.claySoft, border: `1px solid ${T.clay}`, borderRadius: 10, padding: "16px", marginBottom: 20 }}>
                <div style={{ fontFamily: T.font, fontSize: 14, fontWeight: 700, color: T.clay, marginBottom: 6 }}>API key required</div>
                <div style={{ fontFamily: T.font, fontSize: 13, fontWeight: 600, color: T.inkMid, lineHeight: 1.6, marginBottom: 12 }}>Tap the <b>☁</b> button (top right) and add your Anthropic API key to generate practice questions.</div>
                <button onClick={() => setShowSyncPanel(true)} style={{ background: T.clay, color: "#fff", border: "none", borderRadius: 8, padding: "10px 20px", fontFamily: T.font, fontSize: 13, fontWeight: 700, cursor: "pointer" }}>Add API Key →</button>
              </div>
            )}
            {practiceLoading && (
            ...
            {practiceError && !practiceLoading && (
              <div style={{ background: T.claySoft, border: `1px solid ${T.clay}`, borderRadius: 10, padding: "16px", marginBottom: 20 }}>
                <div style={{ fontFamily: T.font, fontSize: 13, fontWeight: 700, color: T.clay, marginBottom: 8 }}>⚠ Error — details below</div>
                <div style={{ fontFamily: "monospace", fontSize: 12, color: T.clay, background: "rgba(139,74,46,0.08)", borderRadius: 6, padding: "10px", marginBottom: 12, wordBreak: "break-all", lineHeight: 1.6, maxHeight: 120, overflowY: "auto" }}>{practiceError}</div>
                <div style={{ display: "flex", gap: 8 }}>
                  <button onClick={generatePracticeQuiz} style={{ flex: 1, background: T.clay, color: "#fff", border: "none", borderRadius: 8, padding: "10px 0", fontFamily: T.font, fontSize: 13, fontWeight: 700, cursor: "pointer" }}>Try Again</button>
                  <button onClick={() => navigator.clipboard?.writeText(practiceError)} style={{ flex: 1, background: T.bgDeep, color: T.inkMid, border: `1px solid ${T.border}`, borderRadius: 8, padding: "10px 0", fontFamily: T.font, fontSize: 13, fontWeight: 700, cursor: "pointer" }}>Copy Error</button>
                </div>
              </div>
            )}
