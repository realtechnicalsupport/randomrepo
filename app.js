...
export default function App() {
  ...
  const SyncPanel = () => (
    <div style={{ position: "fixed", inset: 0, zIndex: 200, display: "flex", alignItems: "flex-end", justifyContent: "center" }} onClick={() => setShowSyncPanel(false)}>
      ...
      <div onClick={e => e.stopPropagation()} style={{ position: "relative", background: T.surface, borderRadius: "16px 16px 0 0", padding: "24px 20px 40px", width: "100%", maxWidth: 560 }}>
        <div style={{ width: 36, height: 4, background: T.border, borderRadius: 2, margin: "0 auto 20px" }} />
        <div style={{ fontFamily: T.font, fontSize: 18, fontWeight: 700, color: T.ink, marginBottom: 6 }}>☁️ Sync Progress</div>
        <div style={{ fontFamily: T.font, fontSize: 13, fontWeight: 600, color: T.inkLight, lineHeight: 1.7, marginBottom: 20 }}>
          Your progress syncs to a private GitHub Gist — invisible to others, accessible on any device.{"\n"}
          To set up: go to <span style={{ color: T.blue }}>github.com/settings/tokens</span>, click "Generate new token (classic)", tick the <b>gist</b> scope, copy the token and paste it below.
        </div>
        <input
          type="password"
          ...
          style={{ width: "100%", padding: "12px 14px", fontFamily: T.font, fontSize: 14, fontWeight: 600, border: `1.5px solid ${T.border}`, borderRadius: 8, background: T.bgDeep, color: T.ink, outline: "none", marginBottom: 12 }}
        />
        {syncToken && (
          <div style={{ fontFamily: T.font, fontSize: 13, fontWeight: 700, color: syncStatus === "error" ? T.clay : syncStatus === "saved" ? T.sage : T.inkLight, marginBottom: 12 }}>
        ...
        {syncToken && (
          <button onClick={async () => { const data = await pullFromGist(syncToken, gistId); if (data) { if (data.completed) setCompleted(data.completed); if (data.recallItems) setRecallItems(data.recallItems); if (data.seenTerms) setSeenTerms(data.seenTerms); setSyncStatus("saved"); setTimeout(() => setSyncStatus("idle"), 2000); } }} style={{ width: "100%", background: T.sage, color: "#fff", border: "none", borderRadius: 8, padding: "12px 0", fontFamily: T.font, fontSize: 14, fontWeight: 700, cursor: "pointer", marginBottom: 8 }}>
            ↓ Pull latest from Gist now
          </button>
        )}
